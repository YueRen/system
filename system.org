...
Run the following command and follow instructions on
...
Put the following into `~/.julia/config/startup.jl` to auto-active envirments.
#+begin_src julia
  import Pkg
...
** snap
#+begin_src bash
  sudo snap install btop chromium gdu gimp libreoffice mpv slack tldr yt-dlp zotero-snap
#+end_src
*** btop
...
* Extras
** libation
For backing up audible audiobooks
#+begin_src bash
  snap install libation
#+end_src
** BambuStudio
For 3d printing
#+begin_src
